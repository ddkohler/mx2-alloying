"""Note: this data was not directly used in the manuscript.
Still potentially useful.
In the interest of posterity, I am including it here.
"""
...
data_dir = here / "ws2 monolayers" / "na-assisted growth"
...
def main():
    ...
    temp = wt.data.from_Solis(data_dir / spectrum_fname.format("reference"))
    temp = temp.split("yindex", [750, 1750])[1]
    temp.copy(name="spectrum", parent=root)
    for sample in ["spot1", "spot2"]:
        temp = wt.data.from_Solis(data_dir / spectrum_fname.format(sample))
        temp = temp.split("yindex", [750, 1750])[1]
    ...
    root.print_tree()
    root.save(here / "ws2_na_assisted.wt5", overwrite=True)
